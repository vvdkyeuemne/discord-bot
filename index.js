...
// ============ Facebook AUTO Download (downr.org) ============
// Lưu cấu hình auto vào fb-settings.json
const FACEBOOK_SETTINGS_FILE = path.join(process.cwd(), 'fb-settings.json');
...
// gọi load 1 lần khi khởi động file
loadFacebookSettings().catch(()=>{});

// phát hiện url facebook
...
async function fetchFacebookPackageFromDownr(rawUrl) {
  ...
  try {
    ...
    // medias
    const medias = Array.isArray(data.medias) ? data.medias.filter(m => m && m.url) : [];
    // caption/title
    const caption = (data.caption || data.title || '').toString();
    // thumbnail: thử data.thumbnail -> data.thumb -> từ medias ảnh đầu tiên
    let thumb = data.thumbnail || data.thumb || data.image || null;
    if (!thumb) {
      const img = medias.find(m => /image/i.test(m.type || '') || /\.(jpg|jpeg|png|webp)(\?|$)/i.test(m.url || ''));
      if (img) thumb = img.url;
    }
    return { caption, thumb, medias };
  } catch (e) {
    console.error('downr fetch error:', e?.message);
    return null;
  }
}
...
client.on('messageCreate', async (msg) => {
  try {
    ...
    await loadFacebookSettings();
    const g = fbSettings.guilds[msg.guild.id] || {};
    const mode = g.mode || 'off';
    if (mode === 'off') return;
    if (mode === 'channel' && g.channelId && g.channelId !== msg.channel.id) return;
