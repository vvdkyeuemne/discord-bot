...
// ===== QUIZ (mini game no-API) =====
const QUIZ_SESS = new Map(); // key = `${gid}:${uid}:${nonce}` -> { correct, msgId, expiresAt }
const quizNow = () => Date.now();
const quizNonce = () => Math.random().toString(36).slice(2) + quizNow().toString(36);
...
function quizRows(gid, uid, nonce, disable = false, labels = []) {
  const row = new ActionRowBuilder().addComponents(
    ...
    new ButtonBuilder().setCustomId(`quiz_ans:${gid}:${uid}:${nonce}:2`).setStyle(ButtonStyle.Secondary).setLabel(labels[2] || 'C').setDisabled(disable),
    new ButtonBuilder().setCustomId(`quiz_ans:${gid}:${uid}:${nonce}:3`).setStyle(ButtonStyle.Secondary).setLabel(labels[3] || 'D').setDisabled(disable),
  );
  return [row];
}

// ===== MILLIONAIRE (Ai là triệu phú) =====
const MILL_SESS = new Map(); // key = `${gid}:${uid}:${nonce}` -> session
const MILL_FILE =
...
// Bank nội bộ cho Ai Là Triệu Phú (Tiếng Việt, không cần API)
const MILL_BANK = [
  // ===== EASY (1–5) =====
  { q: "Thủ đô của Nhật Bản là?", a: ["Osaka","Kyoto","Tokyo","Nagoya"], ok: 2, diff: "easy" },
  { q: "Hành tinh gần Mặt Trời nhất?", a: ["Sao Thủy","Sao Kim","Trái Đất","Sao Hỏa"], ok: 0, diff: "easy" },
  { q: "Nguyên tố có ký hiệu O là?", a: ["Vàng","Oxy","Bạc","Nitơ"], ok: 1, diff: "easy" },
  { q: "Biểu tượng quả táo cắn dở là hãng nào?", a: ["Microsoft","Samsung","Apple","Sony"], ok: 2, diff: "easy" },
  { q: "Bộ truyện có nhân vật Luffy?", a: ["Naruto","One Piece","Bleach","Dragon Ball"], ok: 1, diff: "easy" },
  { q: "Môn thể thao dùng vợt và lưới?", a: ["Bóng đá","Quần vợt","Bóng rổ","Bóng chuyền"], ok: 1, diff: "easy" },
  { q: "Sông nào chảy qua Paris?", a: ["Seine","Danube","Thames","Rhine"], ok: 0, diff: "easy" },
  { q: "Quốc gia vô địch World Cup 2018?", a: ["Đức","Pháp","Brazil","Argentina"], ok: 1, diff: "easy" },
  { q: "Ai phát minh ra bóng đèn?", a: ["Tesla","Einstein","Edison","Newton"], ok: 2, diff: "easy" },
  { q: "Đơn vị tiền tệ của Việt Nam?", a: ["Yên","Đô la","Bảng","Đồng"], ok: 3, diff: "easy" },
  { q: "Ngôn ngữ đánh dấu cho trang web?", a: ["Python","C++","HTML","Java"], ok: 2, diff: "easy" },
  { q: "Trái Đất quay quanh gì?", a: ["Mặt Trời","Mặt Trăng","Sao Kim","Sao Hỏa"], ok: 0, diff: "easy" },
  { q: "Động vật nào kêu 'meo'?", a: ["Chó","Mèo","Bò","Vịt"], ok: 1, diff: "easy" },
  { q: "Nước nào có tháp Eiffel?", a: ["Anh","Pháp","Ý","Tây Ban Nha"], ok: 1, diff: "easy" },
  { q: "Ứng dụng nào thuộc Meta?", a: ["YouTube","TikTok","Instagram","Snapchat"], ok: 2, diff: "easy" },
  { q: "Hành tinh đỏ là?", a: ["Sao Kim","Sao Hỏa","Sao Thổ","Sao Hải Vương"], ok: 1, diff: "easy" },
  { q: "Loại máy tính cầm tay phổ biến?", a: ["Laptop","Máy giặt","Tủ lạnh","Lò nướng"], ok: 0, diff: "easy" },
  { q: "Biển nào nằm giữa châu Âu và châu Phi?", a: ["Caribbean","Đỏ","Địa Trung Hải","Ban Tích"], ok: 2, diff: "easy" },
  { q: "Quốc kỳ Việt Nam có màu chủ đạo nào?", a: ["Xanh","Vàng","Đỏ","Trắng"], ok: 2, diff: "easy" },
  { q: "Thức uống nổi tiếng của Việt Nam?", a: ["Cà phê","Sữa lắc","Soda","Cocoa"], ok: 0, diff: "easy" },
  { q: "Trò chơi xếp khối nổi tiếng?", a: ["Tetris","Pac-Man","Mario","FIFA"], ok: 0, diff: "easy" },
  { q: "Thủ đô của Thái Lan?", a: ["Hà Nội","Bangkok","Kuala Lumpur","Manila"], ok: 1, diff: "easy" },
  { q: "Hệ điều hành di động của Google?", a: ["iOS","HarmonyOS","Android","Windows"], ok: 2, diff: "easy" },
  { q: "Vị chua tự nhiên trong chanh là?", a: ["Axit citric","Axit axetic","Axit lactic","Axit formic"], ok: 0, diff: "easy" },
  { q: "Năm có 366 ngày gọi là?", a: ["Nhuận","Âm","Dương","Chẵn"], ok: 0, diff: "easy" },
  { q: "Pokemon nổi tiếng màu vàng?", a: ["Charmander","Bulbasaur","Pikachu","Squirtle"], ok: 2, diff: "easy" },
  { q: "Mạng xã hội video ngắn nổi tiếng?", a: ["TikTok","Reddit","Facebook","Zalo"], ok: 0, diff: "easy" },
  { q: "Ngôn ngữ chính thức của Trung Quốc?", a: ["Cantonese","Mandarin (Quan thoại)","Tiếng Anh","Nhật"], ok: 1, diff: "easy" },
  { q: "Sơn Tùng M-TP là ai?", a: ["Ca sĩ","Cầu thủ","Diễn viên","Đầu bếp"], ok: 0, diff: "easy" },
  { q: "Hãng sản xuất iPhone?", a: ["Samsung","Apple","Nokia","Huawei"], ok: 1, diff: "easy" },

  // ===== MEDIUM (6–10) =====
  { q: "Vị vua sáng lập nhà Nguyễn?", a: ["Gia Long","Minh Mạng","Thiệu Trị","Tự Đức"], ok: 0, diff: "medium" },
  { q: "Vịnh nào được mệnh danh 'kỳ quan thiên nhiên' ở Việt Nam?", a: ["Vân Phong","Cam Ranh","Hạ Long","Lăng Cô"], ok: 2, diff: "medium" },
  { q: "CPU là viết tắt của?", a: ["Central Processing Unit","Control Program Unit","Central Program Utility","Computer Power Unit"], ok: 0, diff: "medium" },
  { q: "Nhà văn 'Lão Hạc' là ai?", a: ["Nam Cao","Ngô Tất Tố","Vũ Trọng Phụng","Thạch Lam"], ok: 0, diff: "medium" },
  { q: "Bộ ba nguyên màu ánh sáng?", a: ["Đỏ–Lục–Lam","Đỏ–Vàng–Xanh","Lục–Tím–Vàng","Cam–Lam–Lục"], ok: 0, diff: "medium" },
  { q: "Ngày Trái Đất quay quanh Mặt Trời ~ bao nhiêu?", a: ["24h","7 ngày","30 ngày","365 ngày"], ok: 3, diff: "medium" },
  { q: "Dòng sông dài nhất châu Á?", a: ["Mê Kông","Dương Tử (Yangtze)","Hằng (Ganga)","Hoàng Hà"], ok: 1, diff: "medium" },
  { q: "Tác giả 'Những người khốn khổ'?", a: ["Balzac","Victor Hugo","Molière","Zola"], ok: 1, diff: "medium" },
  { q: "Loại sóng dùng cho lò vi sóng?", a: ["Hồng ngoại","Tử ngoại","Vi ba","Âm"], ok: 2, diff: "medium" },
  { q: "Quốc gia có kim tự tháp Giza?", a: ["Ma Rốc","Ai Cập","Algeria","Sudan"], ok: 1, diff: "medium" },
  { q: "Đơn vị đo cường độ dòng điện?", a: ["Volt","Ohm","Ampere","Watt"], ok: 2, diff: "medium" },
  { q: "Ngôn ngữ chính thức của Brazil?", a: ["Tây Ban Nha","Bồ Đào Nha","Anh","Pháp"], ok: 1, diff: "medium" },
  { q: "Nhạc cụ có 88 phím?", a: ["Guitar","Piano","Violin","Sáo"], ok: 1, diff: "medium" },
  { q: "Thủ đô của Úc?", a: ["Sydney","Melbourne","Perth","Canberra"], ok: 3, diff: "medium" },
  { q: "Hợp chất chính của muối ăn?", a: ["NaCl","KCl","NaHCO₃","CaCO₃"], ok: 0, diff: "medium" },
  { q: "Thư viện JS UI do Facebook phát triển?", a: ["Vue","Angular","React","Svelte"], ok: 2, diff: "medium" },
  { q: "Bức tranh 'Mona Lisa' do ai vẽ?", a: ["Van Gogh","Michelangelo","Leonardo da Vinci","Raphael"], ok: 2, diff: "medium" },
  { q: "Động vật có túi nổi tiếng ở Úc?", a: ["Gấu trúc","Kangaroo","Hải cẩu","Đà điểu"], ok: 1, diff: "medium" },
  { q: "Đảo quốc còn gọi là 'xứ sở sương mù'?", a: ["Nhật Bản","Anh","Iceland","Ireland"], ok: 1, diff: "medium" },
  { q: "Hệ điều hành do Linus Torvalds khởi tạo?", a: ["Windows","macOS","Linux","FreeBSD"], ok: 2, diff: "medium" },
  { q: "Nhiệt độ nước sôi ở mực nước biển (°C)?", a: ["90","95","100","105"], ok: 2, diff: "medium" },
  { q: "Cây lúa thuộc nhóm thực vật?", a: ["Họ Hòa thảo","Họ Đậu","Họ Cúc","Họ Na"], ok: 0, diff: "medium" },
  { q: "Nhà bác học phát minh định luật vạn vật hấp dẫn?", a: ["Newton","Galileo","Kepler","Einstein"], ok: 0, diff: "medium" },
  { q: "Siêu kinh điển bóng đá TBN giữa CLB nào?", a: ["Valencia–Sevilla","Real–Barca","Atletico–Sociedad","Betis–Sevilla"], ok: 1, diff: "medium" },
  { q: "Hòn đảo lớn nhất thế giới?", a: ["Borneo","New Guinea","Greenland","Madagascar"], ok: 2, diff: "medium" },

  // ===== HARD (11–15) =====
  { q: "Nguyên tố kim loại lỏng (cùng với brom)?", a: ["Natri","Thủy ngân","Kẽm","Nhôm"], ok: 1, diff: "hard" },
  { q: "Ngôn ngữ KHÔNG thuộc hệ Ấn–Âu?", a: ["Nga","Hindi","Hungary","Bồ Đào Nha"], ok: 2, diff: "hard" },
  { q: "Số Avogadro xấp xỉ?", a: ["6,02×10²³","9,81 m/s²","3,14","1,38×10⁻²³"], ok: 0, diff: "hard" },
  { q: "Chiến dịch Điện Biên Phủ kết thúc năm?", a: ["1952","1954","1956","1960"], ok: 1, diff: "hard" },
  { q: "Nhà toán học cùng Newton khai sinh giải tích?", a: ["Euler","Gauss","Descartes","Leibniz"], ok: 3, diff: "hard" },
  { q: "Hạt quyết định tính chất hóa học?", a: ["Nơtron","Proton","Electron","Nucleon"], ok: 2, diff: "hard" },
  { q: "Độ pH trung tính ở 25°C?", a: ["6","7","8","9"], ok: 1, diff: "hard" },
  { q: "Người đầu tiên bay vào vũ trụ?", a: ["Neil Armstrong","Yuri Gagarin","Buzz Aldrin","Alan Shepard"], ok: 1, diff: "hard" },
  { q: "Tác phẩm KHÔNG của Nam Cao?", a: ["Sống mòn","Chí Phèo","Vợ nhặt","Đời thừa"], ok: 2, diff: "hard" },
  { q: "Thành phần chính khí quyển Trái Đất?", a: ["Oxy","CO₂","Nitơ","Argon"], ok: 2, diff: "hard" },
  { q: "Giao thức bảo mật cho HTTP?", a: ["FTP","SSH","TLS/SSL","SCP"], ok: 2, diff: "hard" },
  { q: "Vi điều khiển Arduino Uno?", a: ["ESP32","ATmega328P","STM32","PIC"], ok: 1, diff: "hard" },
  { q: "Ngôn ngữ truy vấn CSDL quan hệ?", a: ["GraphQL","SQL","NoSQL","Gremlin"], ok: 1, diff: "hard" },
  { q: "Hiện tượng 'El Niño' liên quan?", a: ["Địa Trung Hải","Thái Bình Dương","Ấn Độ Dương","Bắc Băng Dương"], ok: 1, diff: "hard" },
  { q: "Tốc độ ánh sáng xấp xỉ (km/s)?", a: ["150.000","200.000","300.000","450.000"], ok: 2, diff: "hard" },
  { q: "Nhà soạn nhạc 'Giao hưởng số 9'?", a: ["Bach","Beethoven","Mozart","Chopin"], ok: 1, diff: "hard" },
  { q: "Người Việt đầu tiên nhận giải Fields?", a: ["Ngô Bảo Châu","Vũ Hà Văn","Trần Quốc Quân","Chưa có"], ok: 0, diff: "hard" },
  { q: "Loài cây quang hợp CAM?", a: ["Xương rồng","Lúa","Bạch đàn","Cà phê"], ok: 0, diff: "hard" },
  { q: "Vật chất chiếm ~27% vũ trụ?", a: ["Năng lượng tối","Vật chất thường","Vật chất tối","Phản vật chất"], ok: 2, diff: "hard" },
  { q: "Silicon Valley ở bang nào?", a: ["Texas","New York","California","Washington"], ok: 2, diff: "hard" },
  { q: "Nguyên lý bất định của ai?", a: ["Bohr","Heisenberg","Schrödinger","Planck"], ok: 1, diff: "hard" },
  { q: "Điểm sôi etanol (°C)?", a: ["65","78","90","100"], ok: 1, diff: "hard" },
  { q: "Đơn vị công suất âm thanh?", a: ["Bel","Decibel","Newton","Candela"], ok: 1, diff: "hard" },
  { q: "Ngôn ngữ lập trình ra đời sớm nhất?", a: ["Fortran","C","Pascal","Java"], ok: 0, diff: "hard" },
  { q: "Sao có vành rõ nhất?", a: ["Sao Hỏa","Sao Mộc","Sao Thổ","Sao Thiên Vương"], ok: 2, diff: "hard" }
];



// Chọn câu: chỉ dùng bank nội bộ (không API, không dịch)
async function millPickQuestion(step) {
  // 1–5: easy, 6–10: medium, 11–15: hard
  const diff = step <= 5 ? 'easy' : step <= 10 ? 'medium' : 'hard';

  // Lọc bank theo độ khó
  const pool = MILL_BANK.filter(x => x.diff === diff);
  if (!pool.length) {
    // Nếu lỡ thiếu diff nào, fallback mọi câu trong bank
    const any = MILL_BANK[Math.floor(Math.random() * MILL_BANK.length)];
    return { q: any.q, a: [...any.a], ok: any.ok, diff: any.diff || diff };
  }

  // Bốc ngẫu nhiên 1 câu
  const pick = pool[Math.floor(Math.random() * pool.length)];

  // Trộn thứ tự đáp án nhưng vẫn giữ đúng chỉ số đáp án đúng
  const order = [0, 1, 2, 3].sort(() => Math.random() - 0.5);
  const opts = order.map(i => pick.a[i]);
  const okIndex = order.indexOf(pick.ok);

  return { q: pick.q, a: opts, ok: okIndex, diff };
}
...
new SlashCommandBuilder()
  .setName('boss')
  .setDescription('Quản lý Boss/Raid')
  .addSubcommand(sc => sc.setName('start')
    .setDescription('Gọi boss ngay (ngẫu nhiên nếu không chọn)')
    .addStringOption(o => o.setName('id').setDescription('ID boss (slime/dragon/ancient/...)').setRequired(false))
    .addIntegerOption(o => o.setName('hp').setDescription('Ép HP cụ thể').setRequired(false))
  )
  .addSubcommand(sc => sc.setName('status').setDescription('Xem tình hình boss hiện tại'))
  .addSubcommand(sc => sc.setName('auto')
    .setDescription('Bật/tắt tự spawn & giờ sinh')
    .addStringOption(o => o.setName('enable').setDescription('on/off').setRequired(false))
    .addStringOption(o => o.setName('times').setDescription('Giờ, vd: 09:00,13:00,19:00').setRequired(false))
    .addStringOption(o => o.setName('tz').setDescription('Timezone, vd: Asia/Ho_Chi_Minh').setRequired(false))
  ),
...
setTimeout(async () => {
  ...
  if (millNow() >= s.deadline) {
    ...
    // ✅ tích hợp cộng MBBank khi hết giờ
...
if (interaction.commandName === 'capcutauto') {
  ...
  // Giải link rút gọn on.soundcloud.com -> trả về link full soundcloud.com
async function resolveSoundCloudUrl(url) {
  if (!/on\.soundcloud\.com\//i.test(url)) return url;
  try {
    // không follow redirect, chỉ lấy Location
    ...
    console.warn('resolveSoundCloudUrl error:', e?.message || e);
  }
  return url;
}
...
if (interaction.isChatInputCommand() && interaction.commandName === 'playlist') {
  ...
  const ensurePL = (name) => {
    ...
    return { key, list: store[key] };
  };
  const fmtList = (arr, start = 1) =>
    arr.map((u, i) => `${i + start}. <${u}>`).join('\n');

  try {
    // --- /playlist create name:xxx ---
    if (sub === 'create') {
...
if (sub === 'add') {
  ...
  return interaction.editReply(`✅ Đã thêm **${added}** link vào **${name}**:\n${preview}${more}`);
    ...
    if (sub === 'view') {
      ...
      return interaction.reply({
        embeds: [
          new EmbedBuilder()
            .setColor(0xff8800)
            .setTitle(`▶️ Playlist: ${name}`)
            .setDescription(fmtList(slice, (p - 1) * per + 1) || '(trống)')
            .setFooter({ text: `Trang ${p}/${totalPages} • Tổng ${list.length} bài` })
        ],
        ephemeral: true
      });
    }
    ...
    if (sub === 'play') {
      ...
      const name = interaction.options.getString('name', true);
      const { list } = ensurePL(name);
      if (list.length === 0) return interaction.editReply(`(Trống) **${name}** chưa có bài.`);

      // phải ở voice
      const vch = interaction.member?.voice?.channel;
      if (!vch) return interaction.editReply('🎧 Vào voice channel trước đã!');

      // kết nối voice nếu cần
      const state = ensureGuildMusicSC(gid);
      if (!state.connection || state.connection.state.status !== VoiceConnectionStatus.Ready) {
        state.connection = await connectToVoiceSC(vch).catch(() => null);
      }
      if (!state.connection) return interaction.editReply('❌ Không kết nối được voice.');

      // nơi bot sẽ gửi “Now Playing” và nút điều khiển
      state.textChannelId = interaction.channelId;

      // đẩy vào hàng đợi và phát nếu đang rảnh
      state.queue.push(...list);
      if (!state.playing) playNextSC(gid, client);

      return interaction.editReply(`🎶 Đã xếp **${list.length}** bài từ **${name}** vào hàng đợi.`);
    }
    ...
    if (sub === 'shuffle') {
      const name = interaction.options.getString('name', true);
      const { list } = ensurePL(name);
      if (list.length < 2) return interaction.reply('Không đủ bài để xáo trộn.');
...
if (interaction.isChatInputCommand() && interaction.commandName === 'boss') {
  ...
  if (sub === 'start') {
    ...
    const bossId = interaction.options.getString('id') || null;   // để trống = random
    const hp     = interaction.options.getInteger('hp') || null;  // có thể null

    const ok = raidStart(gid, hp, bossId, interaction.guild);
    if (!ok) {
      return interaction.reply({ content: '⚠️ Boss đang tồn tại. Dùng `/boss status`.', ephemeral: true });
    }

    const r2 = ensureRaidGuild(gid);
    return interaction.reply({ embeds: [buildStartEmbed(r2)], components: raidButtons(gid) });
  }

  // /boss status
  if (sub === 'status') {
    if (!r.boss || r.boss.hp <= 0) {
      return interaction.reply({ content: 'Hiện **không có boss** nào.', ephemeral: true });
    }
  ...
  // /boss auto [enable] [times] [tz]
  if (sub === 'auto') {
    if (!interaction.memberPermissions?.has(PermissionFlagsBits.ManageGuild)) {
      return interaction.reply({ content: 'Bạn cần quyền **Manage Server** để chỉnh auto.', ephemeral: true });
    }

    const enStr = (interaction.options.getString('enable') || '').toLowerCase(); // on/off
    const times = interaction.options.getString('times');                        // "09:00,13:00,19:00"
    const tz    = interaction.options.getString('tz');                           // "Asia/Ho_Chi_Minh"

    r.auto = r.auto || { enabled:true, tz:'Asia/Ho_Chi_Minh', times:['09:00','13:00','19:00','21:00'], lastSpawn:0 };
    if (enStr === 'on')  r.auto.enabled = true;
    if (enStr === 'off') r.auto.enabled = false;
    if (times) r.auto.times = times.split(',').map(s => s.trim()).filter(Boolean);
    if (tz)    r.auto.tz    = tz;

    saveRaid();

    return interaction.reply({
      content: `Auto: **${r.auto.enabled ? 'ON' : 'OFF'}** • TZ: \`${r.auto.tz}\` • Times: \`${(r.auto.times||[]).join(', ')}\``,
      ephemeral: true
    });
  }
...
// ====================== RAID / BOSS SYSTEM (single-file block) ======================
// DB path (ưu tiên volume /data trên Railway)
const RAID_FILE =
...
// Chuẩn hoá bản ghi raid cho từng guild
function normalizeRaidGuild(g = {}) {
  return {
    ...
    auto   : g.auto && typeof g.auto === 'object'
              ? g.auto
              : { enabled: true, tz: 'Asia/Ho_Chi_Minh', times: ['09:00','13:00','19:00','21:00'], lastSpawn: 0 },
...
const BOSSES = [
  ...
  { id:'mecha_titan', name:'Mecha Titan', emoji:'🤖', color:0x546e7a,
    ...
    note:'Cỗ máy huỷ diệt – HP lớn, thưởng lớn.' },
...
// scale HP nhẹ theo quy mô server
...
function pickBoss(metaId=null, guild=null){
  ...
  return { id:b.id, name:b.name, emoji:b.emoji, color:b.color, img:b.img, hp, maxHP:hp, reward:b.reward, note:b.note };
}
...
// Roll damage / reward theo level & bảng boss
...
function rollReward(bossReward){
  ...
  return { coins: rng(bossReward.coins[0], bossReward.coins[1]),
           exp  : rng(bossReward.exp[0],   bossReward.exp[1]) };
}
...
function raidStart(gid, hpOrNull=null, bossId=null, guildObj=null){
  ...
  if (Number(hpOrNull)) { meta.hp = meta.maxHP = Number(hpOrNull); }
...
async function pickSendableTextChannel(guild) {
  try {
    ...
    const canSend = (ch) => ch?.type===ChannelType.GuildText &&
    ...
    if (!coll) return null;
    for (const [,ch] of coll) if (canSend(ch)) return ch;
  } catch (_) {}
  return null;
}

// Định dạng giờ phút theo tz
...
client.once(Events.ClientReady, async () => {
  ...
  setInterval(async () => {
    for (const [gid, guild] of client.guilds.cache) {
      ...
      const auto = r.auto || (r.auto = { enabled:true, tz:'Asia/Ho_Chi_Minh', times:['09:00','13:00','19:00','21:00'], lastSpawn:0 });
      ...
      if (should && cool > 4*60*1000) { // cách ~4 phút để tránh spam
        const meta = pickBoss(null, guild);
...
client.on(Events.InteractionCreate, async (itx) => {
  ...
  if (kind === 'raid_status') {
    if (!r.boss || r.boss.hp <= 0) {
      return itx.editReply({ content: 'Hiện **không có boss** nào.' }).catch(()=>{});
    }
    const top = Object.entries(r.contrib || {}).sort((a,b)=>b[1]-a[1]).slice(0,10);
    const lines = await Promise.all(top.map(async ([uid, dmg], i) => {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields({ name:'Bảng xếp hạng', value: lines.join('\n') || 'Chưa có ai tấn công.' })
      .setTimestamp(new Date());
    return itx.editReply({ content: '', embeds:[embed] }).catch(()=>{});
  }

  // Nút Tấn công
  if (kind === 'raid_attack') {
    if (!r.boss || r.boss.hp <= 0) {
      return itx.editReply({ content: 'Boss đã bị hạ hoặc chưa được gọi.' }).catch(()=>{});
    }
    // Lấy ví để tính damage + thưởng
    const uid = itx.user.id;
    const acc = typeof ensureWallet === 'function' ? ensureWallet(uid) : { level:1 };

    const res = raidAttack(gid, acc, uid);
    if (res.error) return itx.editReply({ content: res.error }).catch(()=>{});
    ...
    const rewardStr = `(+${fmtCoins ? fmtCoins(coins) : coins} coins, +${exp} EXP)`;
    await itx.editReply({ content: `⚔️ **${name}** gây **${res.dmg}** dmg. Boss còn **${res.hp}/${res.maxHP} HP**. ${rewardStr}` }).catch(()=>{});

    // Nếu chết thì thông báo chung vào kênh (nếu có)
    if (res.ended) {
      const ch = await pickSendableTextChannel(itx.guild).catch(()=>null);
      if (ch) {
        ch.send({ content: `🏆 **${r.boss.name}** đã bị hạ! Xin chúc mừng các dũng sĩ!` }).catch(()=>{});
      }
    }
  }
});
